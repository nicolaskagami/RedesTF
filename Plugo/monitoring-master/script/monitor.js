...
function Monitor(video_element, url, netflix_element, pinfo_element, jopt, html_geral) {
    console.log("Created Monitor");
    //Get the title
//Get the currentProgress
    this.last_stall_add = false;
    ...
    this.netflix_element_ = netflix_element;
    this.pinfo_element_ = pinfo_element;
    this.jopt_ = jopt;
    this.html_geral_ = html_geral;
    this.url_ = url;
    ...
    this.frames_per_second_ = [];
        this.frames_per_second_mean_ = 0;
        this.frames_len_ = Number.MIN_VALUE;
        this.frames_sum_ = 0;

    this.video_bytes_decoded_per_second_ = [];
    this.video_bytes_decoded_per_second_mean_ = 0;
    this.video_len_ = Number.MIN_VALUE;
        this.video_sum_ = 0;

    this.audio_bytes_decoded_per_second_ = [];
    this.audio_bytes_decoded_per_second_mean_ = 0;
    this.audio_len_ = Number.MIN_VALUE;
        this.audio_sum_ = 0;
...
Monitor.prototype.video_duration = function() {
    console.log("TIT");
//    console.dir(this.video_element_);
    console.log(this.video_element_.videoHeight);
    console.log(this.video_element_.videoWidth);
    console.log("END_TIT");
...
Monitor.prototype.start_stall_monitor = function(check_interval_in_miliseconds) {
    ...
    this.timer_for_stall_counter_= setInterval(function() {
        ...
        {
            self.last_stall = ((new Date().getTime() - self.start_buffering_time_)/1000);
        }
...
Monitor.prototype.store_frame_per_second = function(interval, decoded_frames_per_second) {
    var frame_p = {
       ...
       this.frames_sum_ += frame_p.number_of_frames;
        if (frame_p.current_video_position > this.frames_len_){
        this.frames_len_ = frame_p.current_video_position
        }
    if(this.frames_sum_ > 0){
         this.frames_per_second_mean_ = this.frames_sum_ / this.frames_len_;
        }
...
Monitor.prototype.frame_per_second_mean = function() {
    return this.frames_per_second_mean_;
}
...
Monitor.prototype.store_video_bytes_decoded_per_second = function(interval, video_bytes_decoded_per_second) {
   ...
   this.video_sum_ += video_p.video_bytes;
        if (video_p.current_video_position > this.video_len_){
        this.video_len_ = video_p.current_video_position
        }
    if(this.video_sum_ > 0){
         this.video_bytes_decoded_per_second_mean_ = this.video_sum_ / this.video_len_;
        }
...
Monitor.prototype.video_bytes_decoded_per_second_mean = function() {
        return this.video_bytes_decoded_per_second_mean_;
}
...
Monitor.prototype.store_audio_bytes_decoded_per_second = function(interval, audio_bytes_decoded_per_second) {
    ...
    this.audio_sum_ += audio_p.audio_bytes;
        if (audio_p.current_video_position > this.audio_len_){
        this.audio_len_ = audio_p.current_video_position
        }
    if(this.audio_sum_ > 0){
         this.audio_bytes_decoded_per_second_mean_ = this.audio_sum_ / this.audio_len_;
        }
...
Monitor.prototype.audio_bytes_decoded_per_second_mean = function() {
        return this.audio_bytes_decoded_per_second_mean_;
}

Monitor.prototype.res_history = function(){
    return this.res_history_;
}
...
Monitor.prototype.first_buffer_time_end = function() {
    var buffered_time_interval = [];
    for(var i = 0; i < this.video_element_.buffered.length; i++) {
        return(this.video_element_.buffered.end(i));
        //buffered_time_interval.push({'start' : this.video_element_.buffered.start(i), 'end' : this.video_element_.buffered.end(i)});
    }
...
Monitor.prototype.stop_all_monitoring = function() {
    ...
    console.log(this.last_stall);
    if(this.last_stall != -1)
    {
        this.increment_number_of_stall(this.last_stall);
        console.log("ADD LAST STALL BEFORE QUIT");
    }
}
...
Monitor.prototype.start_skip_monitor = function() {
    var self = this;
//    Object.observe(self.netflix_element_, somethingChanged);
...
//    console.dir(pstream_element);
    ...
    console.dir(jopt);
    html_geral='';
    jopt.find('option').each(function(){
     console.log(this.text);
     console.log(this.value);
     console.log(this.selected); 
         
     if (this.value == "1050"){
        html_geral=html_geral + '<option title="'+ this.text + '" value="' + this.text + '" selected="selected">' + this.text + '</option>';
        this.selected = true;
        $(this).attr('selected', 'selected').parent().focus();
     }else{
             html_geral=html_geral + '<option title="'+ this.text + '" value="' + this.text + '">' + this.text + '</option>';
        this.selected = false;
...
//        jopt.html(html_geral);

        if(self.has_ended()) {
            self.stop_skip_monitor();
        }
        else {
//        if(self.netflix_element_ == null){
//            console.log("DURACAO 1: " + self.video_element_.duration);
//            console.log("RES 1: " + self.video_element_.videoWidth + " x " + self.video_element_.videoHeight);
        if(self.video_element_.videoWidth != self.last_width && self.video_element_.videoWidth != 0 && self.video_element_.videoHeight != self.last_height && self.video_element_.videoHeight != 0){
            self.last_width = self.video_element_.videoWidth;
            self.last_height = self.video_element_.videoHeight;
            self.res_history_ = self.res_history_ + floorFigure(self.video_element_.currentTime, 2) + 's: ' + self.last_width + 'x' + self.last_height + '; ';
                }

//        }else{
//            console.log(self.pinfo_element_[0].firstChild.value);
//            console.log("DURACAO 0: " + self.netflix_element_[0].firstElementChild.firstChild.children[0].duration);
//            console.log("RES 0: " + self.netflix_element_[0].firstElementChild.firstChild.children[0].videoWidth + " x " + self.netflix_element_[0].firstElementChild.firstChild.children[0].videoHeight);
//        }
//            self.jopt_.html(self.html_geral_);
            if(self.video_element_.currentTime != null) {
...
Monitor.prototype.setNetmetricId = function(netmetric, minname) {
    this.netmetric_ = netmetric;
    console.log("MN BEF: " + minname);
    this.minname_ = minname;
    console.log("MN AFT: " + this.minname_);
...
Monitor.prototype.json = function() {
    ...
    console.log("MN HERE: " + this.minname_);
    return_object["minname"] = this.minname_;
    ...
    {
        return_object["video_start_time"] = this.timestamp2;
    }
    ...
    return_object["video_bytes_decoded_per_second_mean"] = this.video_bytes_decoded_per_second_mean();

    return_object["audio_bytes_decoded_per_second"] = this.audio_bytes_decoded_per_second();
    return_object["audio_bytes_decoded_per_second_mean"] = this.audio_bytes_decoded_per_second_mean();
    return_object["time_in_buffer"] = this.time_in_buffer();

    return_object["res_history"] = this.res_history();
    return_object["first_buffer_time_end"] = this.first_buffer_time_end(); //Como o usuario nao vai saltar dentro do video, este representa todo o video bufferizado desde o inicio
    ...
    return_object["frame_per_second_mean"] = this.frame_per_second_mean();
